...
export default function Authentication() {
  ...
  const SignUpCard = () => {
    ...
    const checkPage1 = () => {
      ...
      if (!emailPatternFlag && !passwordFlag && !passwordError && !passwordCheckError) setPage(2);
      }
      //  description: 페이지2에서 회원가입시 검증 함수 //
      const chekPage2 = () => {
    ...
    const onPasswoordCheckIconClickHandler = () => {
      ...
      <div className='auth-card'>
        <div className='auth-card-top'>
          ...
          <div className='auth-card-top-input-container'>
            {page === 1 ? (
                <>
                  <InputBox label='이메일 주소*' type='text' placeholder='이메일 주소를 입력해주세요.' error={emailPatternError || emailDuplicationError} helper={emailPatternError ? '이메일 주소 포맷이 맞지 않습니다.' : emailDuplicationError ? '중복되는 이메일 주소입니다.' : ''} value={email} setValue={setEmail} />
                  ...
                  <InputBox label='비밀번호 확인*' type={showPasswordCheck ? 'text' : 'password'} placeholder='비밀번호를 다시 입력해주세요.' icon={showPasswordCheck ? INPUT_ICON.ON : INPUT_ICON.OFF} buttonHandler={onPasswoordCheckIconClickHandler} error={passwordCheckError} helper={passwordCheckError ? '비밀번호가 일치하지않습니다.' : ''} value={passwordCheck} setValue={setPasswordCheck} />
                </>
               ) : (
                <>
                  <InputBox label='닉네임*' type='text' placeholder='닉네임을 입력해주세요.' error={nicknameError || nicknameDuplicationError} helper={nicknameError ? '닉네임을 입력해주세요.' : nicknameDuplicationError ? '중복되는 닉네임 입니다.' : ''} value={nickname} setValue={setNickname} />
                  <InputBox label='핸드폰 번호*' type='text' placeholder='핸드폰 번호를 입력해주세요.' error={telNumberError || telNumberDuplicationError} helper={telNumberError ? '숫자만 입력해주세요.' : telNumberDuplicationError ? '중복되는 휴대전화번호 입니다.' : ''} value={telNumber} setValue={setTelNumber} />
                  ...
                  <InputBox label='상세 주소*' type='text' placeholder='상세 주소를 입력해주세요.' value={addressDetail} setValue={setAddressDetail} buttonHandler={onAddressIconClickHandler} />
                </>
              )}
          </div>
        </div>
