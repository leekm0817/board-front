...
export default function Main() {
  ...
  const MainTop = () => {
    ...
    //          effect          //
    //  description: 첫 시작시 인기 게시물 데이터 저장 불러오기  //
    useEffect(() => {
  ...
  const MainBottom = () => {
    ...
    const [popularList, setPopulerList] = useState<string[]>([]);
    ...
    const getPopularListResponseHandler = (responseBody: GetPopularListResponseDto | ResponseDto) => {
      ...
      setPopulerList(popularList);
    }
    // description: 최신 게시물 리스트 불러오기 응답 처리 함수 //
    const getCurrentBoardResponseHandler = (responseBody: GetCurrentResponseDto | ResponseDto) => {
      const { code } = responseBody;
    ...
    useEffect(() => {
      getCurrentBoardListRequest(currentSection).then(getCurrentBoardResponseHandler);
    }, [currentSection]);
