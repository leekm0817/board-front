...
const API_DOMAIN = 'http://43.201.14.103:4040/api/v1';
...
const GET_SUARCH_BOARD_LIST_URL = (searchWord: string, relationWord?: string) => relationWord ? `${API_DOMAIN}/board/search/${searchWord}/${relationWord}` : `${API_DOMAIN}/board/search/${searchWord}`;
...
const PATCH_USER_FROFILE_URL = () => `${API_DOMAIN}/user/profile`;

const GET_SIGN_IN_USER_URL = () => `${API_DOMAIN}/user`;
const UPLOAD_FILE = () => `http://43.201.14.103:4040/file/upload`;
...
export const getSearchBoardListRequest = async (searchWord: string, relationWord?: string) => {
  const result = await axios.get(GET_SUARCH_BOARD_LIST_URL(searchWord, relationWord))
  .then((response) => {
...
export const patchProfileImageRequest = async (data: PatchProfileImageRequestDto, token: string) => {
  const result = await axios.patch(PATCH_USER_FROFILE_URL(), data, { headers: { Authorization: `Bearer ${token}` }})
  .then((response) => {
